        ...
        produtos_items = [
            {
                ...
                'embalagem': item.related_estoque.prun_qemb if item.related_estoque else None,
                'embalagemmaster': item.prod_qemb,
                'enviarforcavendas': 'S',
                'pesobruto': item.prod_peso,
                'pesoliq': item.prod_pesoliq,
                'pesovariavel': item.prod_balanca,
                'qtunit': item.related_estoque.prun_qemb if item.related_estoque else None,
                'qtunitcx': item.prod_qemb,
                'revenda': 'S',
                'tipoestoque': 'FR' if item.related_estoque.prun_setordep == '1' or item.related_estoque.prun_setordep == '3' else 'PA',
                'tipomerc': 'L',
                'unidade': item.related_estoque.prun_emb if item.related_estoque else None,
