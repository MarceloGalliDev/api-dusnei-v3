...
class ClienteList(Resource):
    def get(self):
        ...
        clientes_items = [
            {
                ...
                'codcli': str(item.clie_codigo),
                'codcob': item.clie_lpgt_codigo,
                'codfuncultalter': str(item.clie_usualt),
        ...
        response = make_response(json.dumps(clientes_items, ensure_ascii=False, use_decimal=True, indent=4, default=lambda o: o.isoformat() if isinstance(o, (datetime.date, datetime.datetime)) else None), 200)
